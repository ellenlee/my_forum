...
RSpec.describe "Auth", type: :request do
  ...
  let!(:user) {  User.create!( :name => "xxx", :email => "foo@example.org", :password => "12345678", :fb_uid => "1234" ) }
  ...
  describe "login" do
    ...
    it "should login failed if wrong email or password" do
      ...
      expect(response).to have_http_status(401)
      # expect(JSON.parse(response.body))["message"].to eq("Email or Password is wrong")
    end


    it "should login failed if wrong password" do
      post "/api/v1/login", :name => user.name , :email => user.email, :password => "xxx"
